...
Once all hands-on activities are confirmed, for each day first read its mapped
source file (steps below), then build ALL modules at once using the following
structure.

### Source Mapping & Concept Extraction

Before writing each day's content, the AI MUST:

1. Read the mapped source file (or files) from the `input/` folder used for this
   learning path. For this project:

   | Day(s) | Topic | Source file(s) to read |
   |--------|-------|------------------------|
   | 1–2 | Git & Code Review | `input/0-foundations/01-git-and-code-review-discipline.md` |
   | 3–4 | Linux & Networking | `input/0-foundations/02-linux-and-networking-basics.md` |
   | 5–6 | Containers | `input/0-foundations/03-containers.md` |
   | 7–8 | Observability | `input/0-foundations/04-observability-101.md` |
   | 9 | Security + Technical Writing | `input/0-foundations/05-security-hygiene.md` + `07-technical-writing.md` |
   | 10 | AI-Assisted Development | `input/0-foundations/06-ai-assisted-development.md` |

2. Extract the day's core, substantive concepts from the source file — the
   technical detail, not just a restatement of the topic. Do not write generic,
   shallow one-liners. Example of the expected depth: instead of "review quality
   matters," state "review quality and thoroughness measurably decline once a
   change exceeds roughly 200–400 changed lines, and smaller changes are reviewed
   faster and merged more often."

3. From the source file's **Sources / reference links**, also read (fetch) the
   underlying reference URLs where the notes lack detail, to extract and confirm
   the concept. Prefer pulling the actual source URLs from the file over
   inventing links.

4. Distinguish concepts (what something is / how it behaves), best practices
   (what to do / avoid), and real-world examples (a named case, product, or
   event), so each flows into the correct new field below.

5. Track the full list of source URLs used so every concept can be attributed in
   the consolidated `## References` section at the end of the file.
...
- **How It Works:** [2-3 sentences. Explain the mechanism, process, or application. Technical where needed, digestible always. Written in 3rd person.
  - **Concepts:** [Bulleted list of the day's core concepts extracted from the mapped source file. Each bullet is a concrete, technically detailed claim — define key terms, include the specific mechanism/figures/behavior — written in 3rd person. Aim for 4–8 substantive bullets, not generic restatements. Place as nested bullets under How It Works.]
  - **Best Practices:** [Bulleted do's/don'ts extracted from the source file's best-practices or guidance. Each is a concrete, actionable rule, written in 3rd person. Place as nested bullets under How It Works.]
  - **Real-World Example:** [1 concrete example, case study, or named instance drawn from the source file's real-world examples, in 1–2 sentences, 3rd person. Omit if the source has none. Place as a nested bullet under How It Works.]
]
- **Supplemental Reading:**
  - 📄 [Official Documentation — source name]: [URL]
  - 📝 [Blog/Medium Article — source name]: [URL]
  - 📝 [Blog/Medium Article — source name]: [URL]
  > Prefer these from the reference URLs present in the mapped source file. All links are validated before publishing.
...
Add this section as a footer before the final version line. It provides learners with references to the capstone brief and hands-on activities document generated by downstream skills.
...
| Document | Location |
|---|---|
| **Published HTML** | `outputs/[topic-slug]/07-[topic-slug]-module-content.html` — generated by Skill 7 |
| **Capstone Project Brief** | `outputs/[topic-slug]/08-[topic-slug]-capstone.md` — generated by Skill 8 |
| **Hands-On Activities** | `outputs/[topic-slug]/09-[topic-slug]-hands-on-activities.md` — generated by Skill 9 |
```

### References

After the Related Documents table and before the final version line, add a
consolidated `## References` section that lists every source URL used to extract
concepts and best practices across all days. Do not cite sources inline on each
bullet — centralize all attribution here.
...
Include every reference URL that informed the extracted concepts, drawn from the
mapped source files' Sources lists and any reference URLs fetched during
extraction.
...
| **Full Module Content** | All days written at once — core concepts + best practices + real-world example (extracted from mapped source files and their reference URLs), Core Idea / Why It Matters / How It Works, supplemental reading, hands-on activity (if applicable), visual placeholder (if flagged) |
| **References** | Consolidated list of all source URLs used for extraction, with no inline citations |
...
*Skill 3 of 9 — Research & Learning Hub*
*Input comes from → Skill 2: Learning Path Architect (Checkpoint 2 confirmed)*
*Output feeds into → Skill 4: Visual Generator (if flagged), Skill 5: Quiz Generator, Skill 7: HTML Generator, Skill 8: Capstone Project Generator, Skill 9: Hands-On Activity Extractor*
