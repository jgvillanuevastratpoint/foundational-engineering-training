...
This is not left to individual habit. Git and code-review discipline is treated as a tool-agnostic, organization-wide standard: the same baseline applies across every unit — software service (web/mobile), cloud, and data/platform repositories — rather than varying by team or tech stack.
...
Feature flags (also called feature toggles) are commonly paired with trunk-based development. They allow a team to merge code that is not yet finished or not yet ready for users directly into the shared branch, hidden behind a runtime toggle, rather than hiding unfinished work inside a long-lived branch (Unleash, "How To Implement Trunk-Based Development: A Practical Guide"). Since branches under this approach are short-lived by design, a consistent naming convention keeps each one's purpose and scope legible at a glance: feature branches follow `feat/<epic/feature/module>-short-description`, bug-fix branches follow `fix/<ticket_number>/<short-description>`, and documentation-only branches follow `docs/<short-description>` so a reviewer knows before opening the diff that no application code is expected to change — applied the same way whether the repository is a web/mobile service, a cloud service, or a data/platform pipeline.
...
Beyond size, a set of hard controls governs how a change reaches the shared branch, applied the same way across web/mobile, cloud, and data/platform repositories: the main branch is protected from direct pushes for everyone, and every change requires a pull request with dual-side review before it merges — enforced by platform settings rather than convention. Required reviewers per path or product line are declared in-repository (for example, a `CODEOWNERS` file) so the right people are automatically requested and bottlenecks stay visible. A pull request is expected to explain the problem, the approach, how it was tested, the risk, and rollback or mitigation considerations, and to arrive free of unresolved lint errors, unused dependencies, secrets, debug logging, and unrelated refactors; architecture-impacting decisions are captured in an ADR (or equivalent) rather than left implicit, and a PR template standardizes this information so it is not left to individual memory. Automated AI-assisted pre-review (for example, a project-specific `/review` or `/security-review` command) runs before a human reviewer is engaged, so human review time goes toward judgment calls rather than mechanical checks.
...
The convention is enforced automatically rather than left to memory: commit-message validation runs as a git hook at commit time, staged files are linted and formatted before the commit completes, and CI re-runs the same checks as the authoritative source of truth. The same format carries into pull requests — a PR title follows `<type>(<scope>): <description>` because it becomes the squash-merge commit message, so it must be clear, concise, and representative of the complete change, for example `feat(auth): add unit tests for PermissionGuard and AMSSecurityClient` or `fix(navigation): close dropdowns on keyboard focus-out`. Documentation-only changes use the `docs` type — for example `docs(runbook): add rollback steps for service X` — which, unlike `feat` or `fix`, does not trigger a version bump, but still keeps the history explicit about what kind of change occurred and lets reviewers and changelog tooling treat it differently from a code change.
...
- Protect the main branch from direct pushes and require dual-side review before merge, with required reviewers per path declared in-repository (e.g., `CODEOWNERS`).
- Write every PR to explain problem, approach, testing, risk, and rollback/mitigation; keep it free of secrets, debug logging, unused dependencies, and unrelated refactors; capture architecture-impacting decisions in an ADR.
- Run automated AI-assisted pre-review (e.g., a `/review` or `/security-review` command) before requesting human review.
- Use `docs/<short-description>` branches and the `docs` commit/PR-title type for documentation-only changes, so they are identifiable before review and excluded from version bumps.
- Apply the same branch naming, commit-validation, and PR/review controls uniformly across software service (web/mobile), cloud, and data/platform repositories.
